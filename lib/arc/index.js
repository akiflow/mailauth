...
const verifyAS = async (chain, opts) => {
    ...
    // it's possible that the algorithm is not in the right case for the platform, so we execute in lower case and then in uppercase

    let pass
    try {
        pass = crypto.verify(
          signatureHeader.signAlgo === 'rsa' ? signatureHeader.algorithm.toLowerCase() : null,
          canonicalizedHeader,
          publicKey,
          Buffer.from(signatureHeader.parsed?.b?.value, 'base64')
        )
    } catch (e) {
        if (e.code.includes('INVALID_DIGEST')) {
            pass = crypto.verify(
              signatureHeader.signAlgo === 'rsa' ? signatureHeader.algorithm.toUpperCase() : null,
              canonicalizedHeader,
              publicKey,
              Buffer.from(signatureHeader.parsed?.b?.value, 'base64')
            )
