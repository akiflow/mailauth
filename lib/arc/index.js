...
const verifyAS = async (chain, opts) => {
    ...
    const algorithm = signatureHeader.signAlgo === 'rsa' ? signatureHeader.algorithm : null
    // it's possible that the algorithm is not in the right case for the platform, so we find the right case.
    const platformCaseAlgorithm = algorithm ? crypto.getHashes().find(hash => hash.toUpperCase() === algorithm.toUpperCase()) : null

    let pass = crypto.verify(
      platformCaseAlgorithm,
        canonicalizedHeader,
        publicKey,
